...
function renderMap(){
 ...
 el.innerHTML=`<div class="page-heading"><div><h1>Referencias Excel</h1><p>Mapeo agrupado por módulo. Busca un concepto para ver todas sus celdas en los libros configurados.</p></div><div style="display:flex;gap:10px"><button class="ghost-button" id="resetExcelReferences">Restaurar originales</button><button class="primary-button" id="saveExcelReferences">Guardar referencias</button></div></div>
 <section class="card mapping-toolbar"><label>Buscar por concepto<input id="excelReferenceSearch" type="search" placeholder="Ej. Folio SENASICA, versión, director..."></label><label>Módulo<select id="excelReferenceModule"><option value="">Todos</option>${modules.map(m=>`<option value="${m}">Módulo ${m}</option>`).join('')}</select></label><span id="excelReferenceCount"></span></section>
 ...
 <div id="excelReferenceGroups">${modules.map(m=>{const mr=rows.filter(r=>r.module===m);return `<section class="card map-module-group" data-map-module="${m}"><div class="map-module-heading"><div><h2>Módulo ${m} · ${capEsc(mr[0]?.moduleTitle||'')}</h2><p>${mr.length} referencias</p></div></div><div class="map-table-wrap"><table class="usage-matrix"><thead><tr><th>Concepto</th><th>Hoja exacta del Excel</th><th>Origen</th><th>Tipo</th><th>Celda / rango destino</th></tr></thead><tbody>${mr.map(r=>`<tr data-map-row data-search="${capAttr(`${r.concept} ${r.label} ${r.sheet} ${r.exactSheet} ${r.type} M${r.module}`.toLowerCase())}"><td><strong>${capEsc(r.label||r.concept)}</strong><small class="map-field-id">${capEsc(r.id)}</small></td><td>${capEsc(r.sheet)}</td><td>${capEsc(r.source)}</td><td>${capEsc(r.type)}</td><td><input class="excel-reference-input" data-reference-id="${capAttr(r.id)}" value="${capAttr(r.target)}" spellcheck="false"></td></tr>`).join('')}</tbody></table></div></section>`}).join('')}</div><p class="save-message" id="excelReferenceMessage"></p>`;
 const valid=/^[A-Z]{1,3}[1-9]\d*(?::[A-Z]{1,3}[1-9]\d*)?$/;
 const search=document.getElementById('excelReferenceSearch'),moduleFilter=document.getElementById('excelReferenceModule'),count=document.getElementById('excelReferenceCount');
 function applyFilter(){const q=search.value.trim().toLowerCase(),mod=moduleFilter.value;let visible=0;el.querySelectorAll('[data-map-module]').forEach(group=>{let groupVisible=0;group.querySelectorAll('[data-map-row]').forEach(row=>{const show=(!q||row.dataset.search.includes(q))&&(!mod||group.dataset.mapModule===mod);row.hidden=!show;if(show){visible++;groupVisible++;}});group.hidden=groupVisible===0;});count.textContent=`${visible} referencia${visible===1?'':'s'}`;}
 search.addEventListener('input',applyFilter);moduleFilter.addEventListener('change',applyFilter);applyFilter();
