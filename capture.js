...
function renderCaptureControl(c){
 ...
 if(c.type==='checkbox'){const checked=c.blankAsUnchecked ? (c.initial===true) : (c.initial!==false);return `<article class="capture-field"><label>Lista de verificación${coord}</label><label class="check-control"><input type="checkbox" ${checked?'checked':''} data-cap-key="${c.id}"><span>${capEsc(c.checkboxMark||c.label||'Activo')}</span></label></article>`;}
