...
function renderMap(){
 ...
 const rows=allSheets().flatMap(x=>x.sheet.controls.map(c=>({module:x.module,sheet:x.sheet.name,...c,target:saved[c.id]||c.range||c.excelCell||''})));
 el.innerHTML=`<div class="page-heading"><div><h1>Referencias Excel</h1><p>Conexión entre cada campo del portal y su celda o rango en el archivo oficial.</p></div><div style="display:flex;gap:10px"><button class="ghost-button" id="resetExcelReferences">Restaurar originales</button><button class="primary-button" id="saveExcelReferences">Guardar referencias</button></div></div><section class="card" style="padding:16px;margin-bottom:16px"><strong>Regla de seguridad</strong><p style="margin:6px 0 0">Solamente puede modificarse la celda o rango destino. El nombre exacto de la hoja, el campo y el tipo permanecen protegidos.</p></section><section class="card map-table-wrap"><table class="usage-matrix"><thead><tr><th>ID</th><th>Módulo</th><th>Hoja exacta del Excel</th><th>Tipo</th><th>Nombre exacto</th><th>Celda / rango destino</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${capEsc(r.id)}</td><td>${r.module}</td><td>${capEsc(r.sheet)}</td><td>${capEsc(r.type)}</td><td>${capEsc(r.label||'')}</td><td><input class="excel-reference-input" data-reference-id="${capEsc(r.id)}" value="${capEsc(r.target)}" spellcheck="false" style="min-width:130px"></td></tr>`).join('')}</tbody></table></section><p class="save-message" id="excelReferenceMessage"></p>`;
