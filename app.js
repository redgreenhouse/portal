...
function renderExcelColorLegend(){
 const rows=[
  ['legend-pink','Rosa','Imagen o evidencia documental','Permite cargar una imagen.'],
  ['legend-yellow','Amarillo','Texto o fecha','Se muestra como fecha cuando el concepto contiene “Fecha”.'],
  ['legend-green','Verde','Dato Maestro','Se llena automáticamente desde Datos Maestros.'],
  ['legend-sky','Azul cielo','Casilla de estado','Cada clic cambia □ → ✓ → ✗ → NL.'],
  ['legend-red','Rojo','Checkbox independiente','Inicia activo y puede encenderse o apagarse.'],
  ['legend-navy','Azul marino','Semáforo','Permite elegir verde, amarillo o rojo.'],
  ['legend-none','Sin color','Contenido fijo','No genera una zona de captura.']
 ];
 return `<section class="excel-color-reference"><div class="excel-color-reference-title"><strong>Referencia de colores del Excel</strong><span>El color de la plantilla define el tipo de captura en la página LIVE.</span></div><div class="excel-color-reference-scroll"><table><thead><tr><th>Color</th><th>Tipo de dato</th><th>Comportamiento</th></tr></thead><tbody>${rows.map(([cls,color,type,behavior])=>`<tr><td><span class="excel-color-swatch ${cls}"></span><b>${color}</b></td><td>${type}</td><td>${behavior}</td></tr>`).join('')}</tbody></table></div></section>`;
}
function openModule2(){const detail=document.getElementById('moduleDetail');detail.hidden=false;detail.innerHTML=`<div class="module-detail-head"><div><h2>Módulo 2 · Mantenimiento de infraestructura</h2><p>Las 12 hojas se muestran como documentos vivos. Los controles aparecen dentro del formato y conservan el contexto original.</p></div><div><button type="button" class="primary-button m2-export-excel">Generar Excel</button> <button class="ghost-button" data-close-module>Cerrar</button></div></div>${renderExcelColorLegend()}<div class="module-document-list">${RED_DATA.module2.map((doc,index)=>`<article class="excel-sheet-card"><div class="excel-sheet-row"><button class="excel-sheet-head" data-sheet-toggle="${doc.id}"><span class="sheet-index">${String(index+1).padStart(2,'0')}</span><span><strong>${doc.title}</strong><small>Hoja: ${doc.code} · ${doc.type}</small></span><span class="sheet-chevron">⌄</span></button><label class="director-release"><input type="checkbox" data-director-release="M2|${doc.code}" ${isDirectorReleased(`M2|${doc.code}`)?'checked':''}><span>Liberado por Director</span></label></div><div class="excel-sheet-body" id="sheet-${doc.id}" hidden><p class="sheet-description">${doc.description}</p><div class="sheet-action"><b>Acción para la carpeta:</b> ${doc.action} · <b>Frecuencia:</b> ${doc.frequency}</div>${renderModule2DocumentContent(doc)}</div></article>`).join('')}</div>`;
