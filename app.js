...
const RELEASE_STORE_KEY='redGreenhouseDirectorRelease';
let directorRelease=JSON.parse(localStorage.getItem(RELEASE_STORE_KEY)||'{}');
function releaseKey(moduleNumber,sheetName){return `M${moduleNumber}|${sheetName}`;}
function saveDirectorRelease(){localStorage.setItem(RELEASE_STORE_KEY,JSON.stringify(directorRelease));}
function releaseControl(moduleNumber,sheetName){const key=releaseKey(moduleNumber,sheetName),checked=!!directorRelease[key];return `<label class="director-release" title="El Director confirma que esta hoja está lista"><input type="checkbox" data-director-release="${esc(key)}" ${checked?'checked':''}><span>Liberado por Director</span></label>`;}
function bindDirectorRelease(root){root.querySelectorAll('[data-director-release]').forEach(e=>{e.addEventListener('click',ev=>ev.stopPropagation());e.onchange=()=>{directorRelease[e.dataset.directorRelease]=e.checked;saveDirectorRelease();};});}
function openModule2(){const detail=document.getElementById('moduleDetail');detail.hidden=false;detail.innerHTML=`<div class="module-detail-head"><div><h2>Módulo 2 · Mantenimiento de infraestructura</h2><p>Las 12 hojas se muestran como documentos vivos. Los controles aparecen dentro del formato y conservan el contexto original.</p></div><button class="ghost-button" data-close-module>Cerrar</button></div><div class="module-document-list">${RED_DATA.module2.map((doc,index)=>`<article class="excel-sheet-card"><button class="excel-sheet-head" data-sheet-toggle="${doc.id}"><span class="sheet-index">${String(index+1).padStart(2,'0')}</span><span><strong>${doc.code==='MAPA 2.1.2'?'Mapa de Polígonos':doc.title}</strong><small>Hoja: ${doc.code} · ${doc.type}</small></span><span class="sheet-head-actions">${releaseControl(2,doc.code)}<span class="sheet-chevron">⌄</span></span></button><div class="excel-sheet-body" id="sheet-${doc.id}" hidden><p class="sheet-description">${doc.description}</p><div class="sheet-action"><b>Acción para la carpeta:</b> ${doc.action} · <b>Frecuencia:</b> ${doc.frequency}</div>${renderModule2DocumentContent(doc)}</div></article>`).join('')}</div>`;
 detail.querySelector('[data-close-module]').addEventListener('click',()=>detail.hidden=true);bindDirectorRelease(detail);
